...
public class SubscriptionController {
    ...
    @PutMapping("/subscriptions/{id}") public ResponseEntity<SubscriptionResponse> editSub(@PathVariable UUID id, @RequestBody SubscriptionEditRequest req) { return ResponseEntity.ok(svc.editSubscription(id, req)); }
    ...
    @GetMapping("/payments") public ResponseEntity<PageResponse<PaymentResponse>> payments(@RequestParam UUID branchId, @RequestParam(defaultValue="0") int page, @RequestParam(defaultValue="20") int size,
        ...
        return ResponseEntity.ok(svc.getPayments(branchId, page, size, from, to)); }
